...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        updates = [(param_var, self.learning_rate*x) for param_var, x in zip(param_vars, grad_tensors)]
        updates = [(i, i-j) for i, j in updates]
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        updates = self.get_updates(loss_tensor, model.get_param_vars(trainable=True))
        update_ops = [tf.assign(old_var, new_var_or_tensor) for (old_var, new_var_or_tensor) in updates]
        train_generator = tfu.MinibatchIndefinitelyGenerator(train_data, 1, self.shuffle)
        val_generator = tfu.MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        train_losses = []
        val_losses = []
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            batch_train = train_generator.next()
            batch_val = val_generator.next()
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_generator = tfu.MinibatchIndefinitelyGenerator(train_data, self.batch_size, self.shuffle)
        val_generator = tfu.MinibatchIndefinitelyGenerator(val_data, self.batch_size, self.shuffle)
        train_losses = []
        val_losses = []
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
