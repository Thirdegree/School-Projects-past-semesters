...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        return max([(self.getQValue(state, action), action) for action in self.getLegalActions(state)] or [None, None], key = lambda x: x[0])[1]
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature in features:
          self.weights[feature] = self.getWeights()[feature] + self.alpha*diff*features[feature]
